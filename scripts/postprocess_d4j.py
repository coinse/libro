...
def individual_run(proj, bug_id, example_test, injection):
    ...
    repo_path = inject_prefix_rootdir(
        proj, bug_id) if injection else d4j_util.repo_path(proj, bug_id)
...
if __name__ == '__main__':
    ...
    if args.all:
        ...
        test_files = glob.glob(os.path.join(
            GEN_TEST_DIR, f'{args.project}_{args.bug_id}_*.txt'))
        example_tests = []
        res_for_bug = {}
        ...
        results = twover_run_experiment(
            args.project, args.bug_id, example_tests)
